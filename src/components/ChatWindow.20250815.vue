...
export default {
  ...
  data() {
    return {
      ...
      suggestedPromptsInitial: [
        { title: "문서공유", desc: "문서 기반 고객 실시간 상담" },
  ...
  methods: {
    ...
    safeFormat(text) {
      ...
      // 1-3) 헤더 구분선( |---|---| 형태 ) 뒤에 바로 본문이 붙는 경우도 분리
      text = text.replace(/(\n\|?\s*:?-{3,}.*\|\s*\n)(?!\|)/g, "$1\n");
...
.chat-bubble.bot .html-block table {
  display: block;                      /* 표 스스로 스크롤 컨테이너 */
  width: max-content;                  /* 내용만큼 가로 확장 */
  max-width: none;                     /* 버블 폭 제한 해제(표 내부 스크롤용) */
  overflow-x: auto !important;         /* ← 표에서만 가로 스크롤 허용 */
  overflow-y: hidden;
  -webkit-overflow-scrolling: touch;
  margin: 8px 0;
...
/* ✅ 말풍선 스타일 */
.bubble-content {
  padding: 10px 14px; border-radius: 12px; white-space: normal;
...
.chat-bubble.bot .bubble-content { background-color: #eeeeee; color: #000; border-bottom-left-radius: 0; line-height: 1.3;}
