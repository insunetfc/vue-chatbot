<template>
  ...
  <div class="chat-wrapper">
    ...
    <!-- ✨ 추천 말풍선 (입력창 위 고정) -->
    <div
      v-if="messages.length === 0 && suggestedPromptsInitial.length"
    ...
    <!-- ✅ 답변 후 후속 제안(한 줄 칩, 기존 스타일) -->
    <div v-else-if="suggestedPrompts.length" class="suggested-prompts">
      <button
...
export default {
  ...
  methods: {
    ...
    updateFollowupSuggestions({lastUser, botText}) {
      // 1) 서버에서 신호를 내려줄 수도 있음: <<<SUGGEST:["a","b","c"]>>>
      const m = botText.match(/<<<SUGGEST:(\[.*?\])>>>/s);
      if (m) {
        try {
          const arr = JSON.parse(m[1]);
          if (Array.isArray(arr) && arr.length) {
            this.suggestedPrompts = arr.slice(0, 6);
            return;
          }
        } catch(err) {
            console.debug("SUGGEST 파싱 실패:", err);
        }
      }
  
      // 2) 클라이언트 기본 후속 제안(경량 규칙)
      const base = (lastUser || "").replace(/\s+/g, " ").trim();
      const topic = base.length > 0 ? base.slice(0, 32) : "이 내용";
      const canned = [
        `${topic} 표로 요약해 줘`,
        `${topic} 실제 사례 3개만 알려줘`,
        `${topic} 실행 체크리스트로 정리해 줘`,
        `관련 핵심 지표와 벤치마크를 수치로 알려줘`,
        `추가로 읽을 만한 자료 추천해 줘`,
        `내 상황(직무/업종)에 맞게 커스터마이즈해 줘`
      ];
      this.suggestedPrompts = canned;
    },
    ...
    async sendMessage() {
      ...
      // ✅ category 포맷
      const categoryValue = this.buildCategoryMetaString();
