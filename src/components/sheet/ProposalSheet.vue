...
export default {
  ...
  emits: ["close", "send", "select-files", "remove-index", "memo-change", "update-favorites", "update-recent", "open-builder"],
  data() {
    return {
      isOver: false,
      hoverDesc: "",
  ...
  methods: {
    ...
    openBuilder(){
      const cleaned = String(this.memo || "")
    
  
      // 1) 부모(ChatWindow)로 open-builder 이벤트 올림
      this.$emit("open-builder", {
        files: this.uploadedFiles || [],
        selected: this.selectedList,
        outputType: this.selectedOutput || null,
        memo: cleaned
      });
...
.sheet-cta .btn.hermes{
  background: linear-gradient(135deg, #ff9f43, #ff6f00);
  color:#fff; border:none; box-shadow: 0 6px 16px rgba(255,159,67,.25);
}
.sheet-cta .btn.hermes:disabled{ opacity:.6; cursor:not-allowed; }
