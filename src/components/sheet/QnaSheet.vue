      ...
      >
        ...
        <section class="sheet-body">
          ...
          >
            ...
            <div class="dz-title">
              여기로 고객질문에 답할 약관/청약서/설계서를 파일을 드래그 또는
              누르세요
            </div>
            <div class="dz-hint">
              여러개의 파일을 넣을 수 있고 파일이 없는 질문은 채팅창을
              이용하세요
            </div>
            ...
            <div
              ...
              aria-label="파일 유형 선택"
            >
              <button class="dz-pick-btn" @click.stop="pickFromDz('docs')">
                📄 문서 선택
              </button>
              <button class="dz-pick-btn" @click.stop="pickFromDz('images')">
                🖼 이미지 선택
              </button>
              <button class="dz-pick-cancel" @click.stop="closeDzPicker">
                취소
              </button>
            </div>
            <div
              v-if="dzPickerOpen"
              class="dz-picker-mask"
              @click="closeDzPicker"
            />
            <input
              ref="fileInputDocs"
          ...
          <section class="sheet-presets">
            ...
            <section class="qna-config" aria-label="QnA 답변 설정">
              ...
              >
                ...
                <div class="seg-help" v-if="selectedTonePrompt">
                  {{ selectedTonePrompt }}
                </div>
              </div>
              ...
              >
                ...
                <div class="seg-help" v-if="selectedLengthPrompt">
              ...
              >
                ...
                <div class="add-actions">
                  <button type="button" class="small" @click="cancelAddToggle">
                    취소
                  </button>
                  <button
                    type="button"
                ...
                >
                  ...
                  <div class="tog-head">
                    ...
                    <button
                      ...
                      :aria-expanded="
                        isAddToggleOpenIndex === gi ? 'true' : 'false'
                      "
                  ...
                  >
                    <div class="add-row">
                      <label class="add-label" :for="`newToggleTitle-${gi}`"
                        >제목</label
                      >
                      <input
                    ...
                    <div class="add-row">
                      <label class="add-label" :for="`newToggleDesc-${gi}`"
                        >상세내용</label
                      >
                      <textarea
                        :id="`newToggleDesc-${gi}`"
                  ...
                  >
                    ...
                    >
                      ...
                      <!-- 사용자 배지 및 삭제 -->
                      <span v-if="item._user" class="chip-tag user"
                        >사용자</span
                      >
                      <span
                        v-if="selectedToggles.has(item.text)"
                        class="chip-check"
                        >✔</span
                      >
                      <span
                  ...
                  <div class="seg-help" v-if="currentTogglePrompt">
                    {{ currentTogglePrompt }}
                  </div>
                </div>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      toneOptions: [
        {
          text: "친근한",
          desc: "상담사 톤으로 부드럽고 공감 있게",
          sub: "초반응대",
        },
        {
          text: "전문가",
          desc: "보험 전문가처럼 신뢰감 있고 단정하게",
          sub: "상품설명",
        },
        {
          text: "간결한",
          desc: "불필요한 수식어 없이 핵심만",
          sub: "간단답변",
        },
        {
          text: "설득형",
          desc: "고객의 이익 강조, 콜투액션 포함",
          sub: "계약유도",
        },
      ],
      lengthOptions: [
  ...
  computed: {
    ...
    selectedLengthPrompt() {
      const found = this.lengthOptions.find(
        (o) => o.text === this.selectedLength
      );
      return found ? found.desc : "";
    },
    currentTogglePrompt() {
      if (
        !this.lastToggleKey ||
        !this.selectedToggles.has(this.lastToggleKey)
      ) {
      ...
      const item = this.toggleGroups
        .flatMap((g) => g.items)
        .find((i) => i.text === this.lastToggleKey);
  ...
  methods: {
    ...
    saveToggle(gi) {
      ...
      if (!title) {
        this.toast("제목을 입력하세요.");
        return;
      }
      if (!desc) {
        this.toast("상세내용을 입력하세요.");
        return;
      }

      const group = this.toggleGroups?.[gi];
      if (!group) {
        this.toast("그룹을 찾을 수 없습니다.");
        return;
      }

      if ((group.items || []).some((i) => i.text === title)) {
        this.toast("이미 존재하는 항목입니다.");
        return;
      }
      ...
      try {
        ...
        if (Array.isArray(up) && up.length) {
          const baseKeys = new Set(
            (this.toggleGroups?.[0]?.items || []).map((i) => i.text)
          );
          const toAppend = up
    ...
    /* QnA 톤/길이/토글 */
    setTone(key) {
      this.selectedTone = key;
      this.composeMemo();
    },
    setLength(key) {
      this.selectedLength = key;
      this.composeMemo();
    },
    toggleToggle(key) {
    ...
    _onEscClose(e) {
      if (e.key === "Escape") this.closeDzPicker();
    },
    pickFromDz(kind) {
      this.triggerPick(kind);
      this.closeDzPicker();
    },
    resolveEmoji(name = "") {
    ...
    validateAndEmit(listLike) {
      ...
      const validRe =
        this.validExt instanceof RegExp
          ? this.validExt
          : new RegExp(String(this.validExt), "i");

      const baseList = Array.isArray(this.uploadedFiles)
        ? [...this.uploadedFiles]
        : [];
      ...
      for (const f of incoming) {
        if (!validRe.test(f.name)) {
          this.toast(`❌ 지원 안함: ${f.name}`);
          continue;
        }
        if (f.size > perFile) {
          this.toast(`❌ ${this._bytesToMB(perFile)}MB 초과: ${f.name}`);
          continue;
        }
...
.sheet-body {
  ...
  overflow-y: auto;
  flex: 1 1 auto;
  padding-bottom: var(--footer-h, 96px);
  -webkit-overflow-scrolling: touch;
}
...
.dz-picker {
  ...
  z-index: 3;
  animation: dz-pop 0.16s ease-out;
}
.dz-pick-btn,
.dz-pick-cancel {
  padding: 10px 12px;
...
.sheet-cta .btn.primary:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
...
/* 첫 진입 강조 */
.notice-pulse {
  animation: addbtn-pop 0.8s ease-out 1;
}
